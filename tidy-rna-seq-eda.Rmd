...
abundances (TPM), and gene lengths:
...
We manipulate the metadata a little bit, mostly for consistency across
the different pipelines below.
...
Now we start with *tidybulk* The following series of piped operations
turns the *SummarizedExperiment* into a `tidybulk` object. It is also
possible to work directly on the SummarizedExperiment by loading the
*tidySummarizedExperiment* package, with details in implementation.
...
`.sample` string, while features are referred to with `.feature`.
...
code may prove to be more efficient, which is way you would be more
likely to find it in package source code.
...
plot_data %>%
  ...
  geom_text_repel(data=plot_data %>%
            ...
            aes(.sample, logcounts, label=SYMBOL), nudge_x=.5, seed=1) +
  scale_color_identity() +
